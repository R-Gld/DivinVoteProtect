...
public class UtilsVote {
	...
	public void voteIsOkay(Player p, int link)
	{
		...
		dm.incrementCountMonth(p);
		dm.incrementCountAll(p);
		dm.updateLastVote(p, link);
		...
		if(voteParty >= needed - 1)
		{
			...
			Bukkit.broadcastMessage("          reçoit §e§l+300,000 $ §finstantanément !");
			Bukkit.broadcastMessage("");
			...
			if(neededLeft % 20 == 0 || neededLeft < 30)
			{
				...
				Bukkit.broadcastMessage("          reçoive §e§l300,000 $ §finstantanément !");
				Bukkit.broadcastMessage("");
				Bukkit.broadcastMessage("     §f§m-------------------------------------");
			}
		}
	...
	@SuppressWarnings("unused")
	public boolean hasVoted5(Player p)  // www.liste-serveurs-minecraft.org
	{
		String ip = p.getAddress().getAddress().toString().replace("/", "");
	...
	@SuppressWarnings("unused")
	public boolean hasVoted4(Player p) // www.serveursminecraft.org
	{
		...
		try {
			...
			if(Integer.parseInt(body) < 86400) {
				p.sendMessage("§7(§a!§7) §aVote complété et vérifié !");
				return true;
			} else {
				p.sendMessage("§7(§c!§7) §cVote non complété ! Essaie de voter correctement sur ce site de vote !");
	...
	@SuppressWarnings("unused")
	public boolean hasVoted3(Player p) // www.serveurs-minecraft.org
	{
	...
	@SuppressWarnings("unused")
	public boolean hasVoted2(Player p) // www.top-serveurs.net
	{
		String ip = p.getAddress().getAddress().toString().replace("/", "");
		...
		BufferedReader in;
		try {
			String apiLink = main.getConfig().getString("config.apiManager.5");
			apiLink = apiLink.replace("%ip", ip);

			url = new URL(apiLink);
			URLConnection con = url.openConnection();

			con.addRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:25.0) Gecko/20100101 Firefox/25.0"); //éviter error 403

			InputStream instream = con.getInputStream();
			String encoding = con.getContentEncoding();
			encoding = encoding == null ? "UTF-8" : encoding;
			String body = IOUtils.toString(instream, encoding);
			if(p.getName().equalsIgnoreCase("Tikifirst")) p.sendMessage("§e!! 5 => " + body);
